...
import { WorktreeRegistry } from "../workspace/worktrees.js";
...
const worktreeInputSchema = {
  worktree_id: z.string().min(1).optional().describe("Workspace ID from workspace_info. Required for reads when task worktrees are registered; use connectionWorkspaceId to select the original project root. Send it on every task call."),
  expected_branch: z.string().min(1).optional().describe("Refuse the call if that worktree has switched away from this branch."),
};

const workspaceContextOutputSchema = {
  workspaceId: z.string().describe("Workspace selected for this request"),
  connectionWorkspaceId: z.string().describe("Original workspace whose connection authorizes this request"),
};
...
export function createMcpServer(ctx: McpContext): McpServer {
  const worktrees = new WorktreeRegistry(ctx.workspace);
  const selectedResult = (workspace: Workspace, data: object): ToolResult =>
    okStructured({ ...data, workspaceId: workspace.id, connectionWorkspaceId: ctx.workspace.id });
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const workspace = worktrees.resolve(args, { allowDiscovery: true });
        const project = workspace.detectProject();
        const git = gitInfo(workspace.root);
        return selectedResult(workspace, {
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        ...
        return selectedResult(workspace, await workspace.listDirectory(args.path, args));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const workspace = worktrees.resolve(args);
        return selectedResult(workspace, await workspace.readFile(args.path, { startLine: args.start_line, endLine: args.end_line }));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const workspace = worktrees.resolve(args);
        return selectedResult(workspace, await searchWorkspace(workspace, args));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const workspace = worktrees.resolve(args);
        return selectedResult(workspace, gitStatus(workspace));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        ...
        if (!latest) {
          return selectedResult(workspace, { available: false, message: "No execution records yet for this workspace." });
        }
        return selectedResult(workspace, {
          available: true,
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const workspace = worktrees.resolve(args);
        return selectedResult(workspace, { records: readExecutionRecords(workspace.id, args.limit) });
      } catch (error) {
        return mapError(error);
      }
    }
    ...
    async (args, extra) => {
      ...
      try {
        ...
        if (action === "list") {
          ...
          return selectedResult(workspace, { action: "list", items });
        }
